...
def _send_email(to_email: str, subject: str, html_body: str, plain_body: str) -> None:
    ...
    try:
        ...
        with open("/app/logs/smtp_debug.txt", "a") as f:
            f.write(f"SMTP Auth Error: {e}\n")
...
def send_email_change_otp_email(to_email: str, otp: str) -> None:
    """ส่ง OTP ไปยังอีเมลใหม่เพื่อยืนยันว่าเจ้าของสิทธิ์จริงก่อนเปลี่ยน"""
    _send_templated_email(
        to_email,
        subject="รหัสยืนยันการเปลี่ยนอีเมล (OTP) - SmartLPR",
        heading="ยืนยันอีเมลใหม่ของท่าน",
        intro_lines=[
            "มีคำขอเปลี่ยนอีเมลของบัญชี SmartLPR มาใช้อีเมลนี้",
            "กรุณานำรหัส OTP ด้านล่างไปกรอกในหน้าเว็บเพื่อยืนยัน",
        ],
        highlight=otp,
        highlight_caption="รหัส OTP (มีอายุ 10 นาที)",
        accent="brand",
        note_lines=[
            "หากท่านไม่ได้เป็นผู้ขอเปลี่ยนอีเมลนี้ กรุณาเพิกเฉยต่ออีเมลฉบับนี้",
            "บัญชีเดิมของท่านจะยังคงใช้งานได้ตามปกติ",
        ],
    )


def send_email_changed_notification_email(old_email: str, new_email: str) -> None:
    """แจ้งเตือนไปยังอีเมลเดิมว่ามีการเปลี่ยนอีเมลสำเร็จแล้ว เพื่อให้เจ้าของแจ้งแอดมินได้ทันถ้าไม่ได้ทำเอง"""
    _send_templated_email(
        old_email,
        subject="แจ้งเตือน: อีเมลบัญชีของท่านถูกเปลี่ยนแปลง - SmartLPR",
        heading="อีเมลบัญชีถูกเปลี่ยนแปลง",
        intro_lines=[
            "อีเมลสำหรับเข้าสู่ระบบ SmartLPR ของบัญชีนี้เพิ่งถูกเปลี่ยนแปลงสำเร็จแล้ว",
            "เพื่อความปลอดภัย ระบบได้บังคับให้ทุกอุปกรณ์ที่เคยเข้าสู่ระบบไว้ต้องเข้าสู่ระบบใหม่ด้วยอีเมลใหม่",
        ],
        detail_rows=[("อีเมลใหม่", new_email)],
        accent="danger",
        note_lines=["หากท่านไม่ได้เป็นผู้ทำรายการนี้ กรุณาติดต่อผู้ดูแลระบบทันที"],
    )
