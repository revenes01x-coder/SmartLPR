...
def predict(img_input) -> tuple[str, float]:
    ...
    # คำนวณความมั่นใจแบบค่าเฉลี่ย (Arithmetic Mean) ตามมาตรฐานอุตสาหกรรม OCR (ไม่โดน Length Penalty)
    confidence = pred_max_prob.mean().item() if pred_max_prob.numel() > 0 else 0.0
