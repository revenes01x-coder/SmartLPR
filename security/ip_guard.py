...
from smartlpr.config import ALLOW_PRIVATE_IP

def _is_blocked_ip(ip_obj) -> bool:
    """เช็ค private/loopback/link-local ของ IP เดียว (รองรับทั้ง IPv4Address/IPv6Address)"""
    # Helper ฟังก์ชันเช็คไอพีเดี่ยว
    def check_ip(ip) -> bool:
        # 1. บล็อค Loopback อย่างถาวร (127.x.x.x / localhost)
        if ip.is_loopback:
            return True
            
        # 2. ข้ามการบล็อค Private IP / Link Local หาก ALLOW_PRIVATE_IP=true
        if ALLOW_PRIVATE_IP:
            return False
            
        # 3. บล็อคปกติตามมาตรฐาน SSRF Guard
