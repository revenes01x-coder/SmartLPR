...
class WebhookCreate(BaseModel):
    ...
    @classmethod
    def validate_no_emoji(cls, v):
        if isinstance(v, str) and _NO_EMOJI_RE.search(v):
            raise ValueError("ไม่อนุญาตให้ใช้อีโมจิ (Emoji) หรืออักขระพิเศษใน Webhook URL")
        return v
...
class PartnerCameraCreate(BaseModel):
    ...
    delay: int = Field(
        ...
        ge=1,
        le=300,
        description="เวลา delay ในการส่ง event เป็นจำนวนเต็ม (วินาที) (1-300)",
    )
...
class AdminQueueEventItem(BaseModel):
    id: str
    source_event_id: str
    user_id: Optional[str] = None
